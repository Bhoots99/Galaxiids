...
#Display all MO2 vs. SGR data 
s.2.1 <- ggplot(data = tb_data[which(is.na(tb_data$Outlier)),]) +
  geom_point(aes(x = SMR_mass, y = resid, color = Category)) +
  geom_smooth(aes(x = SMR_mass, y = resid, color = ""), method = "lm") +
  theme_classic()+
  scale_color_manual(values = c("darkgrey", "limegreen", "orange", "red")) +
  stat_cor(aes(x = SMR_mass, y = resid), 
           method="pearson",                         
           r.accuracy = 0.01,
           label.x = 0.1, label.y = 0.45,              
           size = 4, color = "black")

s.2.2 <- ggplot(data = tb_data[which(is.na(tb_data$Outlier)),]) +
  geom_point(aes(x = RMR_mass, y = resid, color  = Category)) +
  geom_smooth(aes(x = RMR_mass, y = resid, color = ""), method = "lm") +
  theme_classic()+
  scale_color_manual(values = c("darkgrey", "limegreen", "orange", "red")) +
  stat_cor(aes(x = RMR_mass, y = resid), 
           method="pearson",                          
           r.accuracy = 0.01,
           label.x = 0.3, label.y = 0.45,              
           size = 4, color = "black")

#MMR vs. SGR 
s.2.3 <- ggplot(data = tb_data[which(is.na(tb_data$Outlier)),]) +
  geom_point(aes(x = MMR_mass, y = resid, color = Category)) +
  geom_smooth(aes(x = MMR_mass, y = resid, color = ""), method = "lm") +
  theme_classic()+
  scale_color_manual(values = c("darkgrey", "limegreen", "orange", "red")) +
  stat_cor(aes(x = MMR_mass, y = resid),
           method="pearson",                          
           r.accuracy = 0.01,
           label.x = 1, label.y = 0.45,              
           size = 4, color = "black")

#compile all three rates into a single figure
MR_SGR_all   <- ggarrange( print(s.2.1 + rremove("ylab") + rremove("xlab")), 
                           print(s.2.2 + rremove("ylab") + rremove("xlab")), 
                           print(s.2.3 + rremove("ylab") + rremove("xlab")), 
                           labels = c("SMR", "RMR", "MMR"),
                           ncol = 1, nrow = 3, 
                           align = "v", label.y = 1.01, label.x = 0.075,
                           common.legend = TRUE, 
                           legend = "right")
require(grid)
s.2 <- annotate_figure(MR_SGR_all, left = textGrob("SGR Residuals", rot = 90, vjust = 1, gp = gpar(cex = 1.1)), bottom = textGrob("MR (mg/hr/6.06g)", gp = gpar(cex = 1.1)))
...
top_row <- ggarrange(
  ...
  ncol = 3, labels = c("Nov. MMR", "Nov. RMR", "Nov. SMR"), 
  widths = c(0.28, 0.31, 0.325), heights = c(0.2, 0.5, 0.5),
  align = "v", label.y = 0.97, label.x = c(0.01, 0.1, 0.1),
...
bottom_row <- ggarrange(
  ...
  ncol = 3, labels = c("Mar. MMR", "Mar. RMR", "Mar. SMR"), 
...
Fig3 <- annotate_figure(MR_SGR_months, 
                left = textGrob("SGR Residuals", rot = 90, vjust = 1, gp = gpar(cex = 1.1)), 
